...
export const ExistentialLens = ({
  ...
  Langchain.LangGraph.Task(
    { name: "Call Existential Lens" },
    async (messages: BaseMessageType[]) => {
      ...
      const response = await modelWithTools.stream([{
        ...
        content: `
          ...
          Use Jean-Paul Sartre's philosophical perspectives and focused, analytical approach to inform 
          your responses. You do not need to annouce that this is what you are doing, just respond in that 
          style. Do not be overly obvious about the philosophical element you are following, keep responses 
          resembling normal conversational flow. Limit actions as a response.
